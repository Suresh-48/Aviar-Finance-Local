...
function BorrowerProfileEdit() {
  ...
  const updateBorrower = (values) => {
    ...
    let borrowId = data?.borrowerId?._id ? data?.borrowerId?._id : data?.id;
    Services.UpdateBorrowerDetails(borrowId, values, userId).then((res) => {
      if (res.status === 201) {
    ...
    <div style={{ marginTop: "125px" }}>
      <Formik
        initialValues={{
          ...
          cellPhone: data?.cellPhone ? data?.cellPhone : data?.borrowerId?.cellPhone,
          workPhone: data?.workPhone ? data?.workPhone : data?.borrowerId?.workPhone,
          homePhone: data?.homePhone ? data?.homePhone : data?.borrowerId?.homePhone,
          doorNo: data?.doorNo ? data?.doorNo : data?.borrowerId?.doorNo,
...
export default BorrowerProfileEdit;
